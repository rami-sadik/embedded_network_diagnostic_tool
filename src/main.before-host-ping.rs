...
use csv_report::{
    add_ping_csv_report, add_server_csv_report, add_unsupported_protocol_csv_report,
    create_csv_report_header, save_csv_report,
};
use json_report::{
    add_ping_json_report, add_server_json_report, add_unsupported_protocol_json_report,
    create_json_report, save_json_report,
};
use output::{
    print_header, print_ping_dashboard, print_server_dashboard, print_unsupported_protocol,
};
use ping::ping_host;
use report::{
    add_ping_report, add_server_report, add_unsupported_protocol_report, create_report_header,
    save_report,
};
    ...
    for server in &config.servers {
        match server.protocol.as_str() {
            ...
            "icmp" | "ping" => {
                ...
                add_ping_report(&mut text_report_content, server, &result, total_duration_ms);

                add_ping_json_report(
                    &mut json_report_content,
                    server,
                    result.clone(),
                    total_duration_ms,
                ...
                add_ping_csv_report(&mut csv_report_content, server, &result, total_duration_ms);
            }
            _ => {
